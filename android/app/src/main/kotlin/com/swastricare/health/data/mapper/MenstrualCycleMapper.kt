...
fun MenstrualCycleDto.toDomain(): CycleRecord {
    return CycleRecord(
        id = id,
...
fun CycleRecord.toDto(healthProfileId: String): MenstrualCycleDto {
...
fun MenstrualDailyLogDto.toDomain(): DailyLog {
    return DailyLog(
        id = id,
        cycleId = cycleId,
...
fun DailyLog.toDto(healthProfileId: String): MenstrualDailyLogDto {
    return MenstrualDailyLogDto(
        id = id,
        cycleId = cycleId,
        healthProfileId = healthProfileId,
        date = date.toString(),
...
fun MenstrualSettingsDto.toDomain(): CycleSettings {
...
fun CycleSettings.toDto(healthProfileId: String): MenstrualSettingsDto {
    return MenstrualSettingsDto(
        id = UUID.randomUUID().toString(),
        healthProfileId = healthProfileId,
        averageCycleLength = averageCycleLength,
        averagePeriodLength = averagePeriodLength,
        reminderEnabled = reminderEnabled,
        reminderTime = "$reminderTime:00" // Convert HH:mm to HH:mm:ss
    )
}

// ── Enum Conversion Helpers ──

private fun String.toFlowLevel(): FlowLevel {
...
private fun FlowLevel.toDbValue(): String {
    return when (this) {
        FlowLevel.NONE -> "none"
...
private fun String.toSymptomOrNull(): Symptom? {
    return when (this.lowercase()) {
...
private fun List<Symptom>.toDbValue(): String {
    return this.joinToString(",") { it.toDbValue() }
}

private fun Symptom.toDbValue(): String {
    return when (this) {
...
private fun String.toMood(): Mood? {
    return when (this.lowercase()) {
        "happy" -> Mood.HAPPY
        "calm" -> Mood.CALM
...
private fun Mood.toDbValue(): String {
    return when (this) {
        Mood.HAPPY -> "happy"
        Mood.CALM -> "calm"
